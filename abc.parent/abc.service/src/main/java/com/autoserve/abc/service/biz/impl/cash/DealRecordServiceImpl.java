...
public class DealRecordServiceImpl implements DealRecordService {
    ...
    private final Logger        logger = LoggerFactory.getLogger(DealRecordServiceImpl.class);
    @Resource
    ...
    public BaseResult invokePayment(String innerSeqNo) {
        ...
        switch (dealType) {
            case INVESTER:
                ...
                break;
            case PAYBACK:
                payResult = payMent.transferMoney(innerSeqNo, dealRecords);
                break;
            case PURCHASE:
                payResult = payMent.freeze(innerSeqNo, dealRecords);
                break;
        ...
        //如果需要第三方支付返回则由第三方支付的结果来更新dealState
        if (needInvokeThridPart) {
            return BaseResult.SUCCESS;
        }

        String tradeStatus = EasyPayTradeState.TRADE_FINISHED.getState();
        if (!payResult.isSuccess()) {
            tradeStatus = EasyPayTradeState.TRADE_FAILURE.getState();
        }
        BaseResult modResult = modifyDealRecordState(innerSeqNo, tradeStatus, totalAmount);
        //更新资金操作记录表状态   本地操作在这里更新，第三方调用由web层更新
        ...
        cash.modifyCashRecordState(cashRecord);
        for (int i = 0; !modResult.isSuccess() && i < 3; ++i) {
            //重试3次
            modResult = modifyDealRecordState(innerSeqNo, tradeStatus, totalAmount);
            try {
                Thread.sleep(500);
    ...
    public BaseResult doublePayMentNotify(Map notifyData) {
        ...
        String LoanJsonList;
        try {
            LoanJsonList = URLDecoder.decode(LoanList, "UTF-8");
            ...
            String LoanNo = "";
            if (!("".equals(notifyData.get("LoanNo")) || notifyData.get("LoanNo") == null)) {
    ...
    public BaseResult modifyDealRecordStateWithDouble(Map params) {
        ...
        DealRecordDO dealRecordDo = new DealRecordDO();
        //接口支持批量  故cashSeqNo和innerSeqNo相同 否则此处为outSeqNo
        String seqNo = params.get("OrderNo").toString();
        dealRecordDo.setDrInnerSeqNo(seqNo);
        int newState = DealState.NOCALLBACK.getState();
    ...
    public BaseResult PayBackNotify(Map notifyData) {
        ...
        String OrderNo = "";
        try {
            LoanJsonList = URLDecoder.decode(notifyData.get("LoanJsonList").toString(), "utf-8");
            JSONArray list = JSON.parseArray(LoanJsonList);
            JSONObject LoanJsonListMap1 = (JSONObject) list.get(0);
            String inOrderNo = LoanJsonListMap1.getString("OrderNo");
    ...
    public PageResult<DealRecordDO> queryDealByParams(DealRecordDO dealRecordDO, PageCondition pageCondition,
        ...
        if (count > 0) {
            List<DealRecordDO> result = this.dealRecordDao.findDealByParams(dealRecordDO, pageCondition, startDate,
                    endDate);
