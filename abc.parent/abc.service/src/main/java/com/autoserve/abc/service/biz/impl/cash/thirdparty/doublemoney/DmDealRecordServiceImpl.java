 ...
 */
@Service
public class DmDealRecordServiceImpl implements DealRecordService {

    private final Logger        logger = LoggerFactory.getLogger(DmDealRecordServiceImpl.class);
    @Resource
    ...
    public PlainResult<DealReturn> createBusinessRecord(Deal deal, Callback<DealNotify> callBack) {
        ...
        for (DealRecordDO dealRecordDo : dealRecordDoList) {
            ...
            if (deal.getBusinessType().getType() == 3) {
                String payAccount = dealRecordDo.getDrPayAccount();
                String payUserId = "";
                String payUserType = "";
                if (payAccount.split("\\|").length > 1) {
                    payUserId = payAccount.split("\\|")[0];
                    payUserType = payAccount.split("\\|")[1];
                    AccountInfoDO inAccountInfo = account.queryByAccountMark(Integer.parseInt(payUserId),
                ...
                if (receiveAccount.split("\\|").length > 1) {
                    ...
                    AccountInfoDO inAccountInfo = account.queryByAccountMark(Integer.parseInt(receiveUserId),
    ...
    public BaseResult doublePayMentNotify(Map notifyData) {
        ...
        BaseResult modifyResult = modifyDealRecordState(OrderNo.substring(0, OrderNo.length() - 1), status, mount);
        if (!modifyResult.isSuccess()) {
            result.setSuccess(false);
        } else {
