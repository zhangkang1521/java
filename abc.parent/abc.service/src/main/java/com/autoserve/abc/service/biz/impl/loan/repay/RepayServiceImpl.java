...
public class RepayServiceImpl implements RepayService {
    private static final Log      log = LogFactory.getLog(RepayServiceImpl.class);
     ...
     * 1.4 查询账户信息<br>
     * 2. 查询罚息利率并计算罚息<br>
     * 2.1 如果有罚息，则更新还款计划表中的应还罚息、应还总额字段<br>
     * 2.2 如果有罚息，更新收益计划表中的应还罚息、应还总额字段<br>
     * 3. 创建交易记录<br>
    ...
    public BaseResult repay(int loanId, int repayPlanId, PayType payType, int operatorId) {
        ...
        if (repayPlan == null) {
            ...
            return result.setError(CommonResultCode.BIZ_ERROR, "不能对本期还款计划进行还款");
        }
        ...
        if (repayPlan.getReplaceState()) {
            ...
            BaseResult repayModResult = paymentPlanService.modifyPaymentPlan(repayPlan.getId(), PayState.UNCLEAR,
                    PayState.PAYING, payType, deal.getInnerSeqNo().getUniqueNo()); //TODO CHECK OLD SEQ NO IS REPLACED
         ...
         */
            ...
            for (IncomePlan incomePlan : incomePlanList) {
                // 收益人账户Id
                String receiveAccountId = userAccountMap.get(incomePlan.getBeneficiary() + "|"
                ...
                captitalDetail.setPayAccountId(payAccountNo);
                ...
                interestDetail.setPayAccountId(payAccountNo);
                ...
                if (pulishMoney.compareTo(BigDecimal.ZERO) > 0) {
                    DealDetail fineDetail = new DealDetail();
                    fineDetail.setDealDetailType(DealDetailType.PAYBACK_OVERDUE_FINE);
                    fineDetail.setMoneyAmount(pulishMoney.multiply(Arith.div(incomePlan.getPayCapital(),
                            repayPlan.getPayCapital())));
                    fineDetail.setPayAccountId(payAccountNo);
            ...
            if (takeServeFee) {
                ...
                serveDetail.setDealDetailType(DealDetailType.PLA_SERVE_FEE);
                serveDetail.setPayAccountId(userAccountMap.get(loanee.getUserId() + "|"
                        + loanee.getUserType().getType()));
                serveDetail.setReceiveAccountId(payAccountNo);
                serveDetail.setMoneyAmount(repayPlan.getCollectServiceFee());
                dealDetailList.add(serveDetail);
            }
        ...
        if (!invokeResult.isSuccess()) {
            ...
            throw new BusinessException("还款交易执行失败");
        }
    ...
    public PlainResult<BigDecimal> queryPulishMoney(int repayPlanId) {
        ...
        BigDecimal pulishMoney = computePulishMoney(PaymentPlanConverter.toPaymentPlan(repayPlanDO));
