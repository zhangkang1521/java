...
public class BindingWechat {
    ...
    public JsonBaseVO execute(Context context, Navigator nav, ParameterParser params, TurbineRunData rundata) {
        ...
        String userWechatId = params.getString("userWechatId");
        ...
        if (StringUtil.isBlank(userName) || StringUtil.isBlank(passWord)) {
            ...
            if (securityfromSession == null || securityCode == null
                ...
                if (findResult.isSuccess()) {
                    ...
                    //进行解绑
                    UserService.updateWeChatInfo(userWechatId);
                    //进行绑定
                    UserDO userDO1 = new UserDO();
                    userDO1.setUserId(userId);
                    userDO1.setUserWeChatID(userWechatId);
                    userDO1.setUserIsBound(1);
                    userDO1.setUserFreeLogin(1);
                    System.out.println(userWechatId + "userId");
                    userDao.updateByPrimaryKeySelective(userDO1);
                    ...
                    if (pageResult.getData().size() != 0) {
                        ...
                        if (findResult1.isSuccess()) {
                            ...
                            userDO.setUserIsBound(1);
                            userDO.setUserFreeLogin(1);
                            ...
                            if (pageResult2.getData().size() != 0) {
                                ...
                                if (findResult2.isSuccess()) {
                                    ...
                                    userDO2.setUserWeChatID(userWechatId);
                                    userDO2.setUserIsBound(1);
                                    userDO2.setUserFreeLogin(1);
                                    userDao.updateByPrimaryKeySelective(userDO2);
                                    result.setMessage("绑定成功");
                                    result.setSuccess(true);
                        ...
                        if (pageResult3.getData().size() != 0) {
                            ...
                            if (findResult3.isSuccess()) {
                                ...
                                UserService.updateWeChatInfo(userWechatId);
                                //进行绑定
                                userDO3.setUserId(userId);
                                userDO3.setUserWeChatID(userWechatId);
                                userDO3.setUserIsBound(1);
                                userDO3.setUserFreeLogin(1);
                                userDao.updateByPrimaryKeySelective(userDO3);
                                result.setMessage("绑定成功");
                                result.setSuccess(true);
