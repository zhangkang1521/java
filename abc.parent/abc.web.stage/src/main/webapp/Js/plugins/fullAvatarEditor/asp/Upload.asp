...
'处理原始图片开始------------------------------------------------------------------------>
		...
		Set File = Upload.Files("__source")
...
'<------------------------------------------------------------------------处理原始图片结束
'处理头像图片开始------------------------------------------------------------------------>
		'默认的 file 域的名称：__avatar1,2,3...，可在插件配置参数中自定义，参数名：avatar_field_names
		Dim avatars : avatars = Array("__avatar1", "__avatar2", "__avatar3")
		Dim avatar_length : avatar_length = UBound(avatars)
		Dim i
		For i = 0 To avatar_length
			Set File = Upload.Files(avatars(i))
			'保存的文件名，不包括扩展名
			File.UserSetName = "asp_avatar" & ( i + 1) & "_" & FileName
			'保存的文件扩展名，不含点“.”
			File.Extend = "jpg"
			If File.SaveToFile(SavePath, -1, True) Then
				SuccessNum = SuccessNum + 1
	...
	End If
'<------------------------------------------------------------------------处理头像图片结束
	'upload_url中传递的额外的参数，如果定义的method为get请将下面的Upload.Forms换为Request.QueryString
	userid	 = Upload.Forms("userid")
...
End If
'返回上传结果(json)
Response.Write "{""success"":" & Success & ", ""msg"":""" & Msg & """, ""sourceUrl"":""" & SourceUrl & """, ""avatarUrls"":[" & AvatarUrls & "], ""userid"":" & userid & ", ""username"":""" & username & """}"
%>