    ...
    public JsonBaseVO execute(Context context, Navigator nav,ParameterParser params) {
    	...
    	if(user!=null){
    	 ...
    	 if(userRealName!=null){
    		 ...
    		 //result.setMessage(SafeUtil.hideName(userRealName));
...
//    		 session.setAttribute(userRealName, userRealName);
    	 ...
    	 if(userDocNo!=null){ 
    		 ...
    		 userDO.setUserDocNo(userDocNo);   //设置证件号码   		
    		 //result.setMessage(SafeUtil.hideIDNumber(userDocNo));
    		 result.setMessage(userDocNo);
//    		 session.setAttribute(userDocNo, userDocNo);
    		 BaseResult resu = this.userService.modifyRealAuthInfo(userDO);
			 result.setSuccess(resu.isSuccess());
    	 }
