...
public class Transfer {
	...
	private JsonBaseVO launchTransfer( int userId, int originId, double launchMoney,int investId,String flag) {
        ...
        //-1或null表示转让没有次数限制
        if (limitResult.getData().getConfValue() != null && !"-1".equals(limitResult.getData().getConfValue())) {
            ...
            if (transferResult.getData() != null && StringUtils.isNotBlank(limitResult.getData().getConfValue())) {
                if (transferResult.getData().size() >= Integer.valueOf(limitResult.getData().getConfValue())) {
                	result.setSuccess(false);
                    result.setMessage("当前债权转让次数已经达到" + limitResult.getData().getConfValue() + "次，已经无法转让");
                    return result;
                }
      ...
      //转让期数
        List<PaymentPlanDO> paymentPlanDOList  = paymentPlanService.queryByLoanId(originId).getData();
        int collectPeriod = 0;//已还期数
        for(PaymentPlanDO p:paymentPlanDOList){
        	if(p.getPpPayState()==PayState.CLEAR.state){
        		collectPeriod++;
        	}
        }
        pojo.setTransferPeriod(paymentPlanDOList.size()-collectPeriod); // 转让期数 =总期数-已还期数
