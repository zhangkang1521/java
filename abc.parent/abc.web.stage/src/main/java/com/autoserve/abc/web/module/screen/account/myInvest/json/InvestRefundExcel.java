...
public class InvestRefundExcel {
    ...
    public void MyCreditReceived(IncomePlan incomePlan,InvestSearchJDO investSearchJDO){
    	 PageResult<IncomePlan> pageResult=incomePlanService.queryIncomePlanToDayList(incomePlan,investSearchJDO,
    			 new PageCondition(0,incomePlanService.queryIncomePlanToDayList(incomePlan,investSearchJDO,new PageCondition()).getTotalCount()));	
		List<String> fieldName=Arrays.asList(new String[]{"时间","已回款","已回利息","已回本金","已回罚金","未回款","未回利息","未回本金","总额"});
		...
		for(IncomePlan incomePlan2:pageResult.getData()){
			...
			temp.add(incomePlan2.getCollectFine().toString());
...
//			temp.add(incomePlan2.getPayFine().subtract(incomePlan2.getCollectFine()).toString());
