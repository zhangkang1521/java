...
public class WithdrawMoney {
    ...
    public void execute(Context context, ParameterParser params,Navigator nav) {   	
    	...
    	userIdentity.setUserType(user.getUserType());	
     	...
     	if(money!=null && !"".equals(money) && acc!=null && !"".equals(acc)){
...
// 	     	map.put("FeePercent", "0");
	    	 ...
	    	 * 2.不存在，插入
	    	 */	    	
	    	PlainResult<BankInfoDO> bankResult = bankinfoservice.queryBankInfo(user.getUserId().toString(), CardNo);
...
//		    	bankInfo.setCardStatus(CardStatus.STATE_DISABLE);
		    	...
		    	bankResult = bankinfoservice.queryBankInfo(user.getUserId().toString(), CardNo);
	    	}
	     	map.put("WithdrawMoneymoremore", acc.getAccountMark());
	     	map.put("PlatformMoneymoremore","");
	     	map.put("OrderNo", innerseqno.toString());
...
//	     	map.put("FeePercent", "0");
    	...
    	PlainResult<DealReturn> paramMap = tocashservice.toCashOther(user.getUserId(), user.getUserType(), cashMoney, map);
