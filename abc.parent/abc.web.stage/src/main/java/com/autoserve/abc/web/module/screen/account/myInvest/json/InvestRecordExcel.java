...
public class InvestRecordExcel {
    ...
    public void MyInvestTender(InvestSearchJDO investSearchJDO){
		...
		for(MyInvestTender myInvestTender:pageResult.getData()){
			...
			BigDecimal percent = Arith.calcPercent(myInvestTender.getValidInvest(), myInvestTender.getLoanMoney(), LoanState.valueOf(myInvestTender.getLoanState()));
			temp.add(percent.intValue()+"");
			fieldData.add(temp);
		}
