...
public class DateUtil extends DateUtils {
     ...
     * @return
     */
    public static long calculationAesSuffix(){
		// 1、取得本地时间：
		...
		Double yearDouble=Math.pow((double)year, 2.0);		
		Double monthDouble=Math.pow((double)month, 2.0);
		Double dayDouble=Math.pow((double)day, 2.0);
		//算法：（年^2+月^2+日^2)*时
		BigDecimal buffer=new BigDecimal(yearDouble.toString()).add(new BigDecimal(monthDouble.toString())).add(new BigDecimal(dayDouble.toString()));
		long suffix =buffer.multiply(new BigDecimal(hour)).intValue();
		return suffix;
    }
