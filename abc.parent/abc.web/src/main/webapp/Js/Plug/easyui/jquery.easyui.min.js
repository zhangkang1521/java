...
var _val = box.attr("_value"); if (_val) { if (_val == "" || _val == _3fa) return true;}
...
var _5df = {};
if (_5de != undefined && typeof (_5de) != "string") {
    _5df = $.extend({}, opts.queryParams);
} else {
    //带搜索查询条件
    if (opts.IsSearch == undefined) {
        _5df = $.extend({}, createParam3("SearchForm"));
    }
...
if (name) {
var _method = $(_7fb).attr("_method");
_7ff.attr("name", name);
$(_7ff).attr("_method", _method);
...
var _89b = $("<a href=\"javascript:void(0)\" class=\"datebox-clear\"></a>").html(opts.clearText).appendTo(_898);
_899.click(function(){
    _893.calendar.calendar({
        year: new Date().getFullYear(), month: new Date().getMonth() + 1, current: new Date()
    });
    /*选择今天 给控件赋值 by xdl time:2014-11-4 13:12:42*/
    var date = new Date();
...
//    var jkl= $(_892).parent().prev(".easyui-datetimebox");
    if(!$(_892).hasClass('easyui-datetimebox')){
        $(_892).combo("setValue", time).combo("setText", time);
    }else{
    	time=time+' '+(hour<10 ? ('0'+hour): hour)+':'+(minute<10 ? ('0'+minute): minute)+':'+(second<10 ? ('0'+second): second);
        $(_892).combo("setValue", time).combo("setText", time);
    }
    $(_892).combo("hidePanel");
});
    /*添加清空按钮 按钮事件 by xdl time:2014-11-4 13:12:42 */
_89b.click(function () { $(_892).combo("setValue", "").combo("setText", ""); $(_892).combo("hidePanel"); });
_89a.click(function(){
$(_892).combo("hidePanel");
});
};
...
_89d(this,q);
}},currentText:"Today",closeText:"Close",okText:"Ok",clearText:"Clear",formatter:function(date){
var y=date.getFullYear();
var m=date.getMonth()+1;
var d=date.getDate();
return m+"/"+d+"/"+y;
