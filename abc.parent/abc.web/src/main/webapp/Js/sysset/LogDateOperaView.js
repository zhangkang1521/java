...
var MyAction = {
    Init: function () {
        $("#Grid").datagrid({
            url: "/sysset/json/getOperateLogList.json",
            ...
            columns: [
                [
                    ...
                    {
                        field: "olo_OperateTime", title: "操作时间", width: 150, align: "center"
//                        , formatter: function (value, row, index) {
//                        return convertToDate(value, "yyyy-MM-dd hh:mm:ss");
//                    }
                    },
                    { field: "olo_IP", title: "登录IP", width: 150, align: "center" }
                ]
            ],
            pagination: true,
            singleSelect: false
...
//    },
    Del: function () {
        ...
        $.messager.confirm("提示", "确认要删除选中的操作日志吗？", function (r) {
            if (r) {
                $.AjaxColynJson("/sysset/json/deleteOperateLog.json?", { ids: ids }, function (data) {
                    if (data.success) {
                        Colyn.log("删除成功");
                        MyAction.Init();
    ...
    DelAll: function () {
        $.messager.confirm("提示", "确认要删除全部的操作日志吗？", function (r) {
            if (r) {
                $.AjaxColynJson("/sysset/json/deleteOperateLog.json?", { ids: "all" }, function (data) {
                    if (data.success) {
                        Colyn.log("删除成功");
                        MyAction.Init();
                        $("#Grid").datagrid('clearSelections');
    ...
    Search: function () {
        if ($('#SearchForm').form('validate')) {
            ...
            $.post("/sysset/json/getOperateLogList.json", param, function (data) {
