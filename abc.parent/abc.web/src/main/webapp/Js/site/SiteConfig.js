...
var MyAction = {
    Init: function () {
        $("#RoleGrid").datagrid({
            url: "/site/json/getSiteConfigList.json",
            idField: "configid",
            ...
            animate: true,
            columns: [[
                { title: '参数编码', field: 'confKey', width: 150 },
                { title: '参数名称', field: 'confName', width: 150, align: 'center' },
                { title: '参数描述', field: 'confDesc', align: 'center'},
                { title: '参数值', field: 'confValue', align: 'center', width:150 }
            ]]
        });
        ...
        var addDialog = $.hDialog({
            ...
            submit: function () {
                if ($('#Colyn').form('validate')) {
                    ...
                    $.AjaxColynJson('/site/json/AddSiteConfig.json', param, function (data) {
                        if (data.Success) {
                            Colyn.log("添加成功！");
                            MyAction.Init();
                        }
    ...
    Edit: function () {
        ...
        if (row) {
            var editDialog = $.hDialog({
                href: "/site/SiteConfigEditView?siteKey=" + row.confKey,
                width: 600,
                height: 400,
                iconCls: 'icon-add',
                title: "修改角色",
                ...
                submit: function () {
                    if ($('#Colyn').form('validate')) {
                        var param = $('#Colyn').serializeArray();
                        param = convertArray(param);
                        $.AjaxColynJson('/site/json/SaveSiteConfigEdit.json', param, function (data) {
                            if (data.Success) {
                                Colyn.log("修改成功！")
                                MyAction.Init();
                            }
    ...
    Del: function () {
        var row = MyGrid.selectRow();
        if (row) {            
            $.messager.confirm("删除配置项", "确认要删除选中的配置项吗？", function (r) {
                if (r) {
                    $.AjaxColynJson('/site/json/DelSiteConfig.json', { siteKey: row.confKey }, function (data) {
                        if (data.Success) {
                            Colyn.log("删除成功！")
                            MyAction.Init();
