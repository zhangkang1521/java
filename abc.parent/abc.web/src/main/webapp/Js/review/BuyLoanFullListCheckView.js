...
var getData = function (page, rows) {
    $.ajax({
        ...
        success: function (rows) {
            $('#BuyLoanFullGrid').datagrid('loadData', rows);
        }
    });
};
...
var MyAction = {
    Init: function () {
        $("#BuyLoanFullGrid").datagrid({
            ...
            columns: [[
                ...
                {
                   field: "pro_full_date", title: "满标日期", width: 80, align: "center", formatter: function (value, row, index) {
                       return value;
                   }
                },
                { field: "pro_buy_loan_state", title: "审核状态", width: 120, align: "center"}
            ]],
            pagination: true,
            singleSelect: true
        });
        var p = $('#BuyLoanFullGrid').datagrid('getPager');
        $(p).pagination({
    ...
    Search: function () {
        ...
        $.post( "/review/json/BuyLoanListSearch.json?", param, function (data) {
        	$('#BuyLoanFullGrid').datagrid('loadData', {total : 0,rows : []});
			$("#BuyLoanFullGrid").datagrid("loadData", data);
        }, "json");
    }
};
...
function createSearchParam() {
    var form = {
        ...
        buyLoanFullTimeFrom : $("#pro_full_date_from").val(),
        buyLoanFullTimeTo : $("#pro_full_date_to").val(),
