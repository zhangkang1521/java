...
var MyAction = {
    Init: function () {
        $("#NoviceExclusiveListGrid").datagrid({
            method: "POST",
            url: "/redrewardmanage/json/NewerRewardListView.json",
            ...
            {
                field: "redClosetime", title: "有效日期", width: 100, align: "center",
                formatter: function (value) {
                	if(value != null){
                		return value + "天";
                	}else{
        ...
        var p = $('#NoviceExclusiveListGrid').datagrid('getPager');
        $(p).pagination({
            pageSize: 10,
    ...
    Add: function () {
        ...
        var addDialog = $.hDialog({
            href: "/redrewardmanage/NewerRewardAddView?t=1",
            iconCls: 'icon-add',
            title: "添加个人奖励",
            ...
            buttons: [{
                ...
                handler: function () {
                    if ($('#Colyn').form('validate')) {
                        ...
                        $.AjaxColynJson('/redrewardmanage/json/NewerRewardAddData.json', queryParam, function (data) {
                            if (data.success) {
    ...
    Search: function () {
        ...
        $.post("/redrewardmanage/json/NewerRewardListView.json?t=" + new Date() + "&" + getParam(o), param, function (data) {
            $("#NoviceExclusiveListGrid").datagrid("loadData", data);
        }, "json");
    },
    //启用
    Enable: function () {
		 ...
		 $.messager.confirm("操作提示", "您确定要删除这条记录吗？", function (data) {
	         ...
	         if (data) {
	             $.post("/redrewardmanage/json/RewardDel.json?redId="+row.redId, function (data) {
	                 ...
	                 MyAction.Init();
	                 $("#NoviceExclusiveListGrid").datagrid("clearSelections");
	             }, "json");
	         }//end if
	     });
    }
