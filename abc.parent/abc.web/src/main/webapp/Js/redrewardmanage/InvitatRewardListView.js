...
var MyAction = {
    Init: function () {
        $("#InvitatRewardGrid").datagrid({
            method: "POST",
            url: "/redrewardmanage/json/InvitatRewardListView.json",
        ...
        var p = $('#InvitatRewardGrid').datagrid('getPager');
        $(p).pagination({
            pageSize: 10,
            ...
            buttons: [{
                ...
                handler: function () {
                    //if ($('#Colyn').form('validate')) {
                        ...
                        $.AjaxColynJson('/redrewardmanage/json/InviteRewardAddData.json', queryParam, function (data) {
                            if (data.success) {
    ...
    Search: function () {
        ...
        $.post("/redrewardmanage/json/InvitatRewardListView.json?" + getParam(o), param, function (data) {
            $("#InvitatRewardGrid").datagrid("loadData", data);
        }, "json");
    },
    //启用
    Enable: function () {
        ...
        $.post("/redrewardmanage/json/RewardEnable.json?enable=true&redId="+row.redId, function (data) {
            ...
            $('#InvitatRewardGrid').datagrid('loadData', {total: 0, rows: []});
            MyAction.Init();
        }, "json");
    },
    //停用
		 ...
		 $.messager.confirm("操作提示", "您确定要删除这条记录吗？", function (data) {
	         ...
	         if (data) {
	             $.post("/redrewardmanage/json/RewardDel.json?redId="+row.redId, function (data) {
	                 ...
	                 MyAction.Init();
	                 $("#InvitatRewardGrid").datagrid("clearSelections");
	             }, "json");
	         }//end if
	     });
    }
