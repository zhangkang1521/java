 ...
 * web层 积分兑换红包 的java类
 * 
 * @author fangrui 2014年12月30日 下午8:19:52
 */
public class SuccessRewardListView {
    private static Logger logger = LoggerFactory.getLogger(SuccessRewardListView.class);
    ...
    public JsonPageVO<Red> execute(ParameterParser params) {
        ...
        redSearchDO.setRedType(RedenvelopeType.SCORE_RED.type);
        ...
        if (StringUtils.isNotBlank(searchForm)) {
            try {
                ...
                logger.error("红包信息－积分兑换送红包－搜索查询 查询参数解析出错", e);
            }
        }
