...
public class EditMonBankInfo {
    ...
    @Resource
    MonBankInfoService    bankInfoService;

    public JsonBaseVO execute(Context context, @Param("cuiId") Integer monBankId, ParameterParser params) {
        MonBankInfo monBankInfo = new MonBankInfo();
        monBankInfo.setMonBankId(monBankId);
        ...
        if (StringUtils.isNotBlank(searchForm)) {
            try {
                ...
                logger.error("银行信息管理 修改参数解析出错", e);
            }
        }

        BaseResult result = bankInfoService.modifyMonBankInfo(monBankInfo);
