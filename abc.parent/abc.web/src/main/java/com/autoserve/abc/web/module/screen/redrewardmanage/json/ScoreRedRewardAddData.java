 ...
 */
public class ScoreRedRewardAddData {
    private static Logger logger = LoggerFactory.getLogger(ScoreRedRewardAddData.class);
...
//        }
        ...
        for (Object detailObj : detailArray) {
            ...
            for (int i = 0; i < jsonObj.getIntValue("faceAmt"); i++) {
                ...
                red.setRedType(RedenvelopeType.SCORE_RED);
        ...
        if (redTotalVal != calcuTotal) {
            ...
            if (baseResult.isSuccess()) {
                baseResult.setMessage("积分兑换送红包插入成功");
            } else {
                baseResult.setMessage("积分兑换送红包插入失败");
            }
        }
