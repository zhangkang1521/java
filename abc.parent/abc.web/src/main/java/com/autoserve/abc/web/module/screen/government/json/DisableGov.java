...
public class DisableGov {
    ...
    private static Logger logger = LoggerFactory.getLogger(DisableGov.class);

    public JsonBaseVO execute(@Param("govId") Integer govId) {
        JsonBaseVO vo = new JsonBaseVO();
        logger.info("禁用机构 empId={}, govId={}", LoginUserUtil.getEmpId(), govId);
        BaseResult baseResult = govService.disableGovernment(govId);
        ...
        operateLogDO.setOlContent("禁用了ID为"+ govId +"的机构");//具体操作内容
